...
export default class Elements extends React.Component {
  ...
  renderByRuleName (fragment) {
    ...
    switch (fragment.rule) {
      ...
      case 'youtube':
        var videoSeed = fragment.content + i
        if (!this.state[videoSeed]) this.state[videoSeed] = {width: "100%", height: "0"}
        this.videos[videoSeed] = this.videos[videoSeed] || <YouTube id={videoSeed} videoId={fragment.content} opts={this.state[videoSeed]} onReady={() => this.updateVideoDimensions(videoSeed)}/>
        return this.videos[videoSeed]
      case 'left':
      case 'center':
      case 'right':
      case 'justify':
  ...
  updateVideoDimensions (videoSeed) {
    var obj = {}
    var e = $('#'+videoSeed)
    obj[videoSeed] = {width: e.width(), height: e.width() * 315/560}
    if (this.state[videoSeed] != obj) {
      this.videos[videoSeed] = null
      this.setState(obj)
    }
  }
